...
public class ListRatedUserMapper extends
		Mapper<LongWritable, Text, IntWritable, NullWritable> {

	// movie map to keep the movieId-rating
	private Map<Integer, Integer> ratedUserMap = new HashMap<Integer, Integer>();
	...
	protected void map(LongWritable key, Text value, Context context)
		...
		if (ratedUserMap.get(Integer.parseInt(columns[0])) != null) {
	...
	private void loadUserInMemory(Context context) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				ratedUserMap.put(Integer.parseInt(columns[0]),
